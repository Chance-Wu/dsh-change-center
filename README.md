...
- **Qoder 风格块级操作**(diff 内逐块,默认视图):
  - 块与块之间用空行分隔,块首有 Git 风格**行号区间标记**(`-起,数 +起,数`,如 `-1 +4`),替换行不再显得数字重复,定位一目了然;
  - **浮动操作面板**贴在**当前块开始行的右上侧**,随滚动跟随:显示 `N / M` + **已应用 / 已撤销** 胶囊,含 [↑ 上一块] [编辑] [应用该块/撤销该块] [↓ 下一块];
  - **应用该块 / 撤销该块**:只影响当前块,其余块保持不变(逐块接受);操作成功后**自动跳到下一个块**;点击任意块 = 设为当前块,键盘 ↑/↓ 也可跳转;
  - **编辑**:块内直接改代码 →「保存该块」写回文件(编辑即应用该块;撤销该块会丢弃其编辑);
  - 滚动时面板自动跟踪容器顶部的块(滚到底部选中最后一块);块级写入带 `diskBaseline` 外部修改守卫(与整体应用一致);「应用 / 全部应用」写回时保留块内编辑,不会被原始 after 覆盖。
...
└── client/    conversation.view「变更」标签页 + settings.section「变更中心」
               ...
               · DiffViewer(受控草稿:统一/并排/编辑,浮动块操作面板+行号区间标记+全模式行号+行内标注)
