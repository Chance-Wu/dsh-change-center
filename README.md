...
- **捕获**:监听 `tools/result`,把 `write`/`edit` 结果记为文件变更、`bash` 记为命令记录,自动按 agent turn 分组为变更会话;会话名优先取 agent 标题(`session/title` 事件),否则以「Turn N · HH:MM」命名。
- **Qoder 风格块级操作**(diff 内逐块,默认视图):
  - 每个修改块逐行展示(before/after 行号),块头带「块 N · -X +Y」与 **已应用 / 已撤销** 标记;
  - **应用该块 / 撤销该块**:只影响当前块,其余块保持不变(逐块接受);
  - **编辑**:块内直接改代码 →「保存该块」写回文件(编辑即应用该块;撤销该块会丢弃其编辑);
  - 操作成功后**自动跳到下一个块**;每块 ↑/↓ 按钮或键盘上下箭头可自由跳转,当前块高亮;
  - 块级写入带 `diskBaseline` 外部修改守卫(与整体应用一致);「应用 / 全部应用」写回时保留块内编辑,不会被原始 after 覆盖。
- **Diff 视图**:默认 **统一·块**(逐块操作),可切 **并排** / **编辑**;**所有模式显示行号**(统一/块:删除行 = 修改前行号、插入行 = 修改后行号;并排:左右双栏行号;编辑:行号 gutter 随滚动同步);文件头 `+N -M` + 状态徽标;上方 **AI 变更解释卡**(为什么改 / 影响 / 建议 + 相关文件);unified 行内 finding 标注;>500 行折叠渐进展开;脏状态弱视觉为文件名旁 `●`。
- **变更树**:默认**目录树**(目录可折叠、行统计、全部展开/折叠),可切「按扩展名 `*.ext`」分组;同一文件多次写入**只显示最新一次**(按路径去重);行悬停只显示主操作(应用/重试/回滚/重新应用);**大仓模式**:搜索 / M-A-D 操作过滤 / 路径前缀过滤 + 平铺窗口化渲染,万级变更不卡。
...
- **视图与批量**:「当前」(正在工作的 Turn 的 Focus 卡片,自动跟随)/「会话」(按 今天/昨天/更早 分组的历史时间线);Focus 状态卡(摘要 + `✓ N files changed +X -Y` + 风险文字行,可展开 **Timeline** 与「修改 N 个文件」分解)⇄ Review 完整控制;底部 **sticky Action Dock**(跟随滚动、始终可见):「全部应用」(先 **Prepare 预检**——策略 + hash 守卫写盘前全部跑完,再 Commit,返回 `{applied, skipped, superseded, failed, blocked, prepared}`)、「全部回滚」(返回 `{rolledBack, missing, failed}`,缺快照即无法恢复);批量结果 toast 呈现,应用成功带 **Undo** 入口;有风险(外部修改 / 策略 deny)→「⚠ N 个变更… [查看] [仍然全部应用(force)]」轻确认。
...
└── client/    conversation.view「变更」标签页 + settings.section「变更中心」
               ...
               · DiffViewer(受控草稿:统一·块/并排/编辑,块级 编辑/应用/撤销+全模式行号+行内标注+大文件折叠)
               · ReviewBar(文件导航+操作矩阵) · IntelligencePanel(收进「···/更多」) · RiskSignal(三级信号)
               · TimelineView(会话时间轴,迷你/完整) · summary(会话摘要) · statusMeta(状态视觉)
...
> 同路径最新变更按 `createdAt` + 记录序号稳定决出(同毫秒写入不产生歧义)。
...
- **全部应用** = 对每路径最新待审变更**直接应用**;非待审计入 skipped、被覆盖的旧写入计入 superseded、策略 deny 计入 blocked(`?force=1` 时绕过 deny 门禁与外部修改守卫)。
...
| 逐块接受 | 统一·块 视图点「应用该块 / 撤销该块」 | 只影响当前块,操作后自动跳下一块 |
...
> 当前测试基线:**196 个用例 / 24 个文件**(含真实 HTTP 路由集成测试 `routes-e2e.spec.ts`、行为契约 `contracts.spec.ts`)。
>
